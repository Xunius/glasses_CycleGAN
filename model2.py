...
class ConvBlock(nn.Module):
    def __init__(self, c_in, c_out, kernel_size, stride, padding, norm, use_act):
        ...
        elif norm == 'switchable':
            layers.append(SwitchNorm2d(c_out))
...
class ResDownBlock(nn.Module):
    def __init__(self, c_in, c_out, kernel_size, stride, padding, norm):
        ...
        self.layers = nn.Sequential(
                ConvBlock(c_in, c_out, kernel_size=1, stride=1, padding=0,
                    norm=norm, use_act=True),
                ConvBlock(c_out, c_out, kernel_size=kernel_size, stride=stride,
                    padding=padding, norm=norm, use_act=False)
                )
...
class Generator(nn.Module):
    def __init__(self, c_in, n_features=64, n_resblocks=9, n_strides=3, dropout=0.2):
        nn.Module.__init__(self)
        ...
        for ii in range(n_strides):
            ...
            layers.append(
                    ResDownBlock(cii, cii2, kernel_size=3, stride=2, padding=1, norm='bn'))
        ...
        for ii in range(n_resblocks):
            layers.append(ResBlock(c0, norm='switchable'))
            layers.append(nn.LeakyReLU(0.1, True))
            #layers.append(nn.ReLU())
            layers.append(nn.Dropout2d(dropout))
        ...
        for ii in range(n_strides):
            ...
            layers.append(DeconvBlock(cii, cii2, kernel_size=3, stride=2, padding=1, norm='switchable', use_act=True))
...
class DiscBlock(nn.Module):
    def __init__(self, c_in, dropout):
        ...
        self.layers = nn.Sequential(
                ...
                ConvBlock(128, 256, kernel_size=4, stride=2, padding=1, norm='bn', use_act=True),
                nn.Dropout2d(dropout),
                ConvBlock(256, 512, kernel_size=4, stride=2, padding=1, norm='bn', use_act=True),
                #nn.Dropout2d(dropout),
...
class Discriminator(nn.Module):
    def __init__(self, c_in, n_strides, dropout):
        nn.Module.__init__(self)
        ...
        for ii in range(n_strides):
            sii = 2*(ii+1)
            layerii = nn.Sequential(
                    nn.MaxPool2d(sii, sii),
                    DiscBlock(c_in, dropout),
                    nn.Flatten(),
                    nn.Sigmoid(),
                    )
            self.dis_list.append(layerii)

    def forward(self, x):

        res = []
        for layer in self.dis_list:
            yii = layer(x)
            res.append(yii)
...
class CycleGAN(nn.Module):
    def __init__(self, config):
        ...
        self.dis_X = Discriminator(config['img_channels'], config['n_strides'], config['dropout'])
        self.dis_Y = Discriminator(config['img_channels'], config['n_strides'], config['dropout'])
        ...
        self.dis_opt = torch.optim.RMSprop(list(self.dis_X.parameters()) +\
        ...
        self.gen_opt = torch.optim.RMSprop(list(self.gen_X2Y.parameters()) +\
    ...
    def update_gen(self, img_X, img_Y, fake_X, fake_Y, config):
        ...
        # cycle loss
        if config['lambda_cycle'] >0 and np.random.rand() < config['p_cycle']:
...
if __name__=='__main__':
    ...
    mod = Generator(3, 64, 8, 3)
    dis = Discriminator(3, 3, 0.1)
    y = mod(x)
    y2 = dis(y)
